...
const levelSoundAt={target:-Infinity,stop:-Infinity};
const signalSoundAt={buy:-Infinity,sell:-Infinity};
...
const notificationKinds={buy:{label:'AL ADAYI',color:'#49dfae'},sell:{label:'SAT SİNYALİ',color:'#ff7185'},warning:{label:'RİSK UYARISI',color:'#ffd166'},info:{label:'BİLDİRİM',color:'#83aaff'}};
...
const levelText=(price,entry)=>positive(price)&&positive(entry)?money(price)+' ('+pct((price/entry-1)*100)+')':'Belirlenemedi';
...
function validatePosition(p) {return p && ['string','number'].includes(typeof p.id) && String(p.id).length<150 && canonical(symbolOf(p))===symbolOf(p) && ['amount','quantity','entry'].every(k=>positive(p[k])) && (p.stop===null || positive(p.stop)&&p.stop<p.entry) && (p.target===null || positive(p.target)&&p.target>p.entry) && [p.fee??0,p.slippage??0].every(x=>num(x)&&x>=0&&x<=.05) && Paper.validExtras(p);}
function validateHistory(p) {return p && typeof p.coin==='string' && canonical(p.coin)===p.coin && positive(p.entry)&&positive(p.exit)&&num(p.percent)&&typeof p.date==='string'&&p.date.length<100 && (p.pnl===undefined||num(p.pnl)) && Paper.validExtras(p);}
...
function renderSoundStatus(){
  ...
  byId('sound-status').textContent=!soundEnabled?'Ses kapalı; yazılı bildirimler devam eder.':ready?'Ses açık · AL ve SAT sinyalleri farklı 10 saniyelik seslerle, diğer bildirimler kısa nötr sesle bildirilir.':'Ses tercihin açık ve kayıtlı. Tarayıcı sesi başlatmak için bir dokunuş bekliyor; SESİ ETKİNLEŞTİR düğmesine basabilirsin.';
}
...
function playLevelAlarm(kind){
  if(!soundEnabled||!audioContext||audioContext.state!=='running')return false;
  if(Date.now()-levelSoundAt[kind]<9500)return false;
  try{
    const start=audioContext.currentTime,frequencies=kind==='stop'?[880,440]:[784,988];
    for(let i=0;i<20;i++){
      const oscillator=audioContext.createOscillator(),gain=audioContext.createGain(),at=start+i*.5;
      oscillator.type=kind==='stop'?'square':'sine';oscillator.frequency.value=frequencies[i%2];
      gain.gain.setValueAtTime(0,at);gain.gain.linearRampToValueAtTime((kind==='stop'?.22:.16)*soundVolume/100,at+.025);gain.gain.setValueAtTime((kind==='stop'?.22:.16)*soundVolume/100,at+.23);gain.gain.exponentialRampToValueAtTime(.001,at+.36);
      oscillator.connect(gain);gain.connect(audioContext.destination);oscillator.start(at);oscillator.stop(at+.38);oscillator.onended=()=>{oscillator.disconnect();gain.disconnect();};
    }
    lastSoundAt=Date.now();levelSoundAt[kind]=lastSoundAt;return true;
  }catch(_){byId('sound-status').textContent='Seviye alarmı çalınamadı; tarayıcının ses iznini kontrol et.';return false;}
}
function playSignalAlarm(kind){
  if(!soundEnabled||!audioContext||audioContext.state!=='running')return false;
  if(Date.now()-signalSoundAt[kind]<9500)return false;
  try{
    const start=audioContext.currentTime,notes=kind==='buy'?[523,659,784,659]:[880,740,587,440];
    for(let i=0;i<20;i++){
      const oscillator=audioContext.createOscillator(),gain=audioContext.createGain(),at=start+i*.5;
      oscillator.type=kind==='buy'?'sine':'sawtooth';oscillator.frequency.value=notes[i%notes.length];
      gain.gain.setValueAtTime(0,at);gain.gain.linearRampToValueAtTime((kind==='buy'?.17:.2)*soundVolume/100,at+.025);gain.gain.setValueAtTime((kind==='buy'?.17:.2)*soundVolume/100,at+.24);gain.gain.exponentialRampToValueAtTime(.001,at+.37);
      oscillator.connect(gain);gain.connect(audioContext.destination);oscillator.start(at);oscillator.stop(at+.39);oscillator.onended=()=>{oscillator.disconnect();gain.disconnect();};
    }
    lastSoundAt=Date.now();signalSoundAt[kind]=lastSoundAt;return true;
  }catch(_){byId('sound-status').textContent='AL/SAT sesi çalınamadı; tarayıcının ses iznini kontrol et.';return false;}
}
...
function showNotification(message,kind='info',symbol=null,sound='normal'){
  ...
  if(document.hidden){unread[kind]++;updateTabIndicator();}if(sound==='target'||sound==='stop')playLevelAlarm(sound);else if(sound==='buy'||sound==='sell')playSignalAlarm(sound);else if(sound!=='none')playAlertSound();
}
function alertOnce(key,status,message,kind='info',sound='normal') {const prev=state.alertStates.get(key);state.alertStates.set(key,status);if(!status||status===prev||!byId('alerts-enabled').checked)return;const cooldownKey=key+'|'+status,now=Date.now();if(now-(notificationCooldowns.get(cooldownKey)??-Infinity)<90000)return;notificationCooldowns.set(cooldownKey,now);for(const[k,at]of notificationCooldowns)if(now-at>600000)notificationCooldowns.delete(k);const symbol=message.match(/^([A-Z0-9]{1,17}\/TRY)(?=\s|$)/)?.[1]||null;state.alerts.unshift({at:new Date().toISOString(),message,kind,symbol});state.alerts=state.alerts.slice(0,30);renderAlerts();showNotification(message,kind,symbol,sound);}
function notifyHighBuy(s){
  for(const key of s.frame_keys){const frame=s.frames[key],eligible=frame?.checks_passed>=4&&isFresh(quoteFor(s.coin.symbol,s.coin));
    alertOnce('buy-checks:'+s.coin.symbol+'|'+key,eligible?'qualified':'',s.coin.symbol+' · AL adayı · '+frameName(key)+' '+(frame?.checks_passed??0)+'/5 koşul · '+s.score+'/100 teknik puan. Eksik koşulları ve riski incele; kazanç garantisi değildir.','buy','buy');
  }
}
...
function switchPage(page) {state.page=page;document.querySelectorAll('.page').forEach(n=>n.classList.toggle('active',n.id==='page-'+page));document.querySelectorAll('.nav').forEach(n=>n.classList.toggle('active',n.dataset.page===page));byId('title').textContent={home:'Bugün ne yapmalıyım?',radar:'Teknik radar',watchlist:'Takip ettiklerim',market:'Piyasa hareketi',positions:'Sanal portföyüm',performance:'Kâr/zarar ve emirler',exits:'Satış takibi'}[page];byId('page-label').textContent='COINPILOT TR';byId('sidebar').classList.remove('open');if(page==='exits'){renderExitTracking();loadExitChecks();}if(page==='watchlist')renderFavorites();if(page==='performance')renderPerformance();scrollPageTop();requestAnimationFrame(()=>{redrawAll();scrollPageTop();});}
...
function toggleFavorite(symbol){state.favorites=state.favorites.includes(symbol)?state.favorites.filter(s=>s!==symbol):[...state.favorites,symbol];try{store(keys.favorites,state.favorites);}catch(e){notice(e.message);}renderFavorites();document.querySelectorAll('[data-favorite]').forEach(n=>{n.textContent=(state.favorites.includes(n.dataset.favorite)?'★ Takipte':'☆ Takip et');});}
function favoriteButton(symbol){const b=btn('', 'favorite',()=>toggleFavorite(symbol));b.dataset.favorite=symbol;b.textContent=state.favorites.includes(symbol)?'★ Takipte':'☆ Takip et';return b;}
function renderFavorites(){
  byId('favorites').replaceChildren(...state.favorites.map(s=>btn('★ '+s,'favorite',()=>analyzeMarket(s))));
  const list=byId('favorites-page-list');if(!list)return;
  list.replaceChildren(...(state.favorites.length?state.favorites.map(symbol=>{const card=el('article','panel watch-card'),main=el('div');main.append(el('h3','',symbol),quoteBlock(symbol));const actions=el('div','watch-actions');actions.append(btn('ANALİZ ET','',()=>analyzeMarket(symbol)),btn('TAKİPTEN ÇIKAR','sell',()=>toggleFavorite(symbol)));card.append(main,actions);return card;}):[el('div','panel empty','Henüz takip ettiğin coin yok. Piyasa Hareketi veya Teknik Radar sayfasından “Takip et” düğmesini kullanabilirsin.')]))
}
...
function signalCard(s){
  ...
  card.append(el('p','summary-text',s.summary.join(' ')),el('p','muted',s.risk_ok?(s.stop_adjusted?'Teknik stop risk kuralıyla yaklaştırıldı: hedef/stop oranı en az 2:1 ve zarar en fazla %5.':'Stop / hedef mesafesi filtresi uygun; hedef/stop oranı en az 2:1 ve zarar en fazla %5.'):'Hedef veya geçerli risk kontrollü stop planı bulunamadı.'));
...
function renderRadar(scanning=false,unavailable=false){byId('radar-grid').replaceChildren(...(state.signals.length?state.signals.map(signalCard):[el('div','panel empty',scanning?'Tarama sürüyor; sonuçlar geldikçe burada görünecek.':unavailable?'Mum verisi alınamadığı için analiz tamamlanamadı. Bu, uygun coin olmadığı anlamına gelmez.':'Başarıyla analiz edilen paritelerde en az 3/5 koşulu sağlayan aday yok.')]));byId('home-radar').replaceChildren(...(state.signals.length?state.signals.slice(0,3).map(s=>{const c=el('article','panel compact-signal');c.append(btn(s.coin.symbol,'link',()=>showDetail(s)),quoteBlock(s.coin.symbol,s.coin),el('p','summary-text',s.summary.join(' ')));return c;}):[el('p','muted',scanning?'Radar taranıyor…':unavailable?'Radar verisi eksik; uygun aday olmadığı sonucu çıkarılamaz.':'Analiz edilen paritelerde aday yok.')]));byId('setup-count').textContent=state.signals.length;state.signals.forEach(notifyHighBuy);redrawAll();}
...
async function loadRadar(force=false,generation=radarGeneration){clearTimeout(radarTimer);if(state.scope==='favorites'&&!state.favorites.length){state.signals=[];renderRadar();byId('radar-time').textContent='Takip listene önce bir coin ekle.';return;}const params={frames:state.frames.join(','),limit:state.scope==='favorites'?'250':state.scope};if(state.scope==='favorites')params.symbols=state.favorites.join(',');if(force)params.force='1';byId('scan-button').disabled=true;
  let delay=60000;try{const d=await api('/api/radar',params);if(generation!==radarGeneration)return;state.signals=d.items||[];for(const s of state.signals){const key=s.coin.symbol+':'+state.frames.join(','),value=Object.values(s.frames).map(f=>f.checks_passed).join('/'),previous=state.scanStates.get(key);if(previous!==undefined&&previous!==value)alertOnce('scan:'+key,value,s.coin.symbol+' teyit değişimi: '+previous+' → '+value,'info','none');state.scanStates.set(key,value);}renderRadar(d.scanning,!!d.error||(d.errors||[]).length>0);byId('radar-time').textContent=(d.phase==='retrying'?'Başarısız istekler yeniden deneniyor: ':d.scanning?'Taranıyor: ':d.coverage==='unavailable'?'Analiz yapılamadı: ':d.coverage==='partial'?'Kısmi sonuç: ':'Tamamlandı: ')+(d.successful??(d.scanned-(d.errors||[]).length))+'/'+d.total+' başarılı analiz · '+state.signals.length+' aday'+(d.updated_at?' · '+timeText(d.updated_at):'');byId('scan-errors').textContent=d.error||((d.errors||[]).length?'Verisi alınamayan '+d.errors.length+' parite (otomatik yeniden denenecek): '+d.errors.map(x=>x.symbol+' ('+x.message+')').join(' · '):'');delay=d.scanning?2000:d.coverage==='unavailable'?10000:60000;}catch(e){if(generation===radarGeneration){state.signals=[];renderRadar(false,true);byId('radar-time').textContent=e.message+' Sonraki deneme otomatik.';delay=10000;}}finally{if(generation===radarGeneration){byId('scan-button').disabled=false;radarTimer=setTimeout(()=>loadRadar(false,generation),delay);}}}
...
function metric(id,value,note,known=true){const node=byId(id),noteNode=byId(id.replace('-pnl','-note'));node.textContent=known?money(value):'Fiyat bekleniyor';node.className=known?(value>=0?'positive':'negative'):'muted';if(noteNode)noteNode.textContent=note;}
function emptyTableRow(columns,message){const row=el('tr'),cell=el('td','muted',message);cell.colSpan=columns;row.append(cell);return row;}
function localDay(value){const local=String(value||'').match(/^\d{2}\.\d{2}\.\d{4}/)?.[0];if(local)return local;const d=new Date(value);return Number.isFinite(d.getTime())?d.toLocaleDateString('tr-TR'):'';}
function renderPerformance(){
  if(!byId('performance-open-pnl'))return;
  let openCost=0,openValue=0,openComplete=true;for(const p of state.positions){openCost+=p.amount;const value=positionValue(p);if(value===null)openComplete=false;else openValue+=value;}
  const realized=state.history.filter(h=>num(h.pnl)),realizedPnl=realized.reduce((sum,h)=>sum+h.pnl,0),realizedCost=realized.reduce((sum,h)=>sum+(num(h.amount)?h.amount:0),0),today=new Date().toLocaleDateString('tr-TR'),todayRows=realized.filter(h=>localDay(h.closedAt||h.recordedAt||h.date)===today),todayPnl=todayRows.reduce((sum,h)=>sum+h.pnl,0),openPnl=openValue-openCost,totalKnown=openComplete,total=realizedPnl+openPnl,wins=realized.filter(h=>h.pnl>0).length,losses=realized.filter(h=>h.pnl<0).length,flats=realized.length-wins-losses,winRate=realized.length?wins/realized.length*100:null;
  metric('performance-open-pnl',openPnl,state.positions.length+' açık pozisyon',openComplete);metric('performance-realized-pnl',realizedPnl,realized.length+' sonuç kaydı');metric('performance-today-pnl',todayPnl,todayRows.length+' bugünkü kapanış');metric('performance-total-pnl',total,'Gerçekleşen + açık tahmini',totalKnown);
  const summary=byId('performance-summary');summary.replaceChildren();const grid=el('div','performance-summary-grid');[['Kazanma oranı',winRate===null?'Henüz sonuç yok':winRate.toFixed(1)+'%'],['Kazanç / kayıp / başa baş',wins+' / '+losses+' / '+flats],['Satılan kayıtlı maliyet',money(realizedCost)],['Açık pozisyon maliyeti',money(openCost)]].forEach(([label,value])=>{const item=el('div');item.append(el('small','',label),el('b','',value));grid.append(item);});summary.append(grid,el('p','muted','Kazanma oranı yalnızca net K/Z tutarı bulunan sanal satış kayıtlarından hesaplanır. Parçalı satışların her biri ayrı sonuçtur; küçük örneklem gelecekteki başarıyı göstermez.'));
  const groups=new Map();const group=symbol=>{if(!groups.has(symbol))groups.set(symbol,{symbol,sales:0,cost:0,pnl:0,wins:0,losses:0,open:0,openPnl:0,openKnown:true});return groups.get(symbol);};
  for(const h of state.history){const g=group(h.coin);g.sales++;if(num(h.amount))g.cost+=h.amount;if(num(h.pnl)){g.pnl+=h.pnl;if(h.pnl>0)g.wins++;else if(h.pnl<0)g.losses++;}}
  for(const p of state.positions){const g=group(symbolOf(p)),value=positionValue(p);g.open++;if(value===null)g.openKnown=false;else g.openPnl+=value-p.amount;}
  const coinRows=[...groups.values()].sort((a,b)=>b.pnl-a.pnl).map(g=>{const row=el('tr');row.append(el('td','',g.symbol),el('td','',String(g.sales)),el('td','',money(g.cost)),el('td',g.pnl>=0?'positive':'negative',money(g.pnl)),el('td','',g.wins+' / '+g.losses),el('td','',String(g.open)),el('td',g.openKnown?(g.openPnl>=0?'positive':'negative'):'muted',g.openKnown?money(g.openPnl):'Fiyat bekleniyor'));return row;});byId('performance-by-coin').replaceChildren(...(coinRows.length?coinRows:[emptyTableRow(7,'Henüz pozisyon veya gerçekleşen sanal satış yok.')]));
  const orderPositions=state.positions.filter(p=>p.sellOrder),orders=orderPositions.map(p=>{const o=p.sellOrder,row=el('tr'),pending=o.status==='pending';row.append(el('td','',symbolOf(p)),el('td',pending?'pending-order':'muted',pending?'Bekliyor':'İptal edildi'),el('td','',timeText(o.createdAt)),el('td','',levelText(o.price,p.entry)),el('td','',(o.quantity??p.quantity).toLocaleString('tr-TR',{maximumFractionDigits:10})+' adet'),el('td','',o.replay?'Açık':'Kapalı'));return row;});byId('performance-orders').replaceChildren(...(orders.length?orders:[emptyTableRow(6,'Açık veya iptal edilmiş sanal limit emri yok.')]));byId('pending-order-count').textContent=orderPositions.filter(p=>p.sellOrder.status==='pending').length+' bekleyen';
  const completed=state.history.map(h=>{const row=el('tr'),tone=num(h.pnl)?(h.pnl>=0?'positive':'negative'):'';row.title=h.executionNote||'';row.append(el('td','',h.date||timeText(h.closedAt)),el('td','',h.coin),el('td','',executionLabel(h.method)),el('td','',money(h.amount)),el('td','',num(h.quantity)?h.quantity.toLocaleString('tr-TR',{maximumFractionDigits:10}):'—'),el('td','',money(h.entry)+' → '+money(h.exit)),el('td',tone,money(h.pnl)),el('td',h.percent>=0?'positive':'negative',pct(h.percent)));return row;});byId('performance-completed').replaceChildren(...(completed.length?completed:[emptyTableRow(8,'Henüz gerçekleşen sanal satış yok.')]));byId('completed-order-count').textContent=completed.length+' kayıt';
}
function renderPortfolio(){let cost=0,value=0,complete=true;for(const p of state.positions){cost+=p.amount;const v=positionValue(p);if(v===null)complete=false;else value+=v;}
  ...
  for(const id of ['home-positions','position-page-list']){byId(id).replaceChildren(...(state.positions.length?state.positions.map(positionCard):[el('div','panel empty','Henüz açık sanal işlem yok.')]));}
  ...
  renderExitTracking();renderPerformance();
}
function positionCard(p){const symbol=symbolOf(p),q=quoteFor(symbol),value=positionValue(p),gain=value===null?null:value-p.amount,result=gain===null?null:gain/p.amount*100,card=el('article','position');card.dataset.positionId=String(p.id);card.append(el('h3','',symbol),quoteBlock(symbol),el('div','levels','Giriş '+money(p.entry)+' · Tutar '+money(p.amount)+' · Adet '+p.quantity.toLocaleString('tr-TR',{maximumFractionDigits:10})),el('b','value '+(gain>=0?'positive':'negative'),money(value)),el('b',gain>=0?'positive':'negative','Tahmini net K/Z: '+money(gain)+' · '+pct(result)),el('div','levels','Hedef '+levelText(p.target,p.entry)+' · Stop '+levelText(p.stop,p.entry)),el('small','', 'Plan periyodu: '+exitFrames(p).map(frameName).join(' · ')+' · Yüzdeler giriş fiyatına göredir.'));
  ...
  let status='İzleniyor',key='';if(isFresh(q)){if(positive(p.stop)&&q.price<=p.stop){status='Stop seviyesi aşıldı';key='stop';}else if(positive(p.target)&&q.price>=p.target){status='Hedef seviyesine ulaştı';key='target';}else if(positive(p.stop)&&q.price<=p.stop*1.01){status='Stop seviyesine %1 mesafede';key='near-stop';}else if(positive(p.target)&&q.price>=p.target*.99){status='Hedefe %1 mesafede';key='near-target';}if(key==='near-stop'||key==='near-target')alertOnce('position:'+p.id,key,symbol+' · '+status);}else status='Fiyat gecikmiş / bekleniyor; değer son kotasyondur.';
  if(p.riskWarning)card.append(el('p','position-status stale','Giriş uyarısı: '+p.riskWarning));
  card.append(positionChartPanel(p),positionTools(p));
  card.append(el('p','position-status',status));const exitInfo=exitStatus(p);card.append(el('p','position-status '+exitInfo.tone,'Satış takibi: '+exitInfo.label));if(state.page!=='exits')card.append(btn('SATIŞ TAKİBİ','link',()=>switchPage('exits')));const close=btn(state.closing.has(p.id)?'DOĞRULANIYOR…':'PARÇALI / TAM SAT','sell',()=>closePosition(p.id));close.disabled=state.closing.has(p.id);card.append(close);return card;}
let dashboardBusy=false;
async function loadDashboard(){if(dashboardBusy)return;dashboardBusy=true;try{const d=await api('/api/dashboard',{},20000);Object.values(d.quotes).forEach(rememberQuote);byId('live-status').textContent=d.live?'WebSocket · 3 sn ekran yenileme':'REST · fiyat zamanı kontrol edilir';byId('live-dot').classList.toggle('online',d.live);await observePositions();await checkAutoSales();paintQuotes();renderPortfolio();renderMarket(d);loadPositionPaths();if(state.marketSymbol&&!marketBusy&&Date.now()-marketAt>60000)analyzeMarket(state.marketSymbol,true);}catch(e){byId('live-status').textContent='Bağlantı bekleniyor';byId('live-dot').classList.remove('online');paintQuotes();}finally{dashboardBusy=false;}}
...
function tradePlan(s,inputs){const{amount,fee,slip}=inputs,q=s.coin;if(!isFresh(q))throw Error('Güncel fiyat doğrulanamadı; yeniden dene.');const base=positive(q.ask)?q.ask:q.price,entry=base*(1+slip),target=positive(s.target)&&s.target>entry?s.target:null,rawStop=positive(s.stop)&&s.stop<entry?s.stop:null,reward=target?(target-entry)/entry*100:null,riskCap=reward?Math.min(5,reward/2):5,riskFloor=entry*(1-riskCap/100),stop=rawStop?Math.max(rawStop,riskFloor):target?riskFloor:null,risk=stop===null?null:(entry-stop)/entry*100,stopAdjusted=positive(rawStop)&&stop>rawStop||!rawStop&&positive(stop);const quantity=amount/(entry*(1+fee)),atTarget=target===null?null:quantity*target*(1-slip)*(1-fee)-amount,atStop=stop===null?null:quantity*stop*(1-slip)*(1-fee)-amount;const warnings=[];if(!s.all_frames_passed)warnings.push('Seçili periyotlarda 5/5 teyit yok.');if(s.sell_setup)warnings.push('SAT sinyali var.');if(stopAdjusted)warnings.push('Teknik stop, risk kuralıyla '+pct(-risk)+' seviyesine yaklaştırıldı.');if(target===null)warnings.push('Geçerli hedef belirlenemedi; hedef takibi olmayacak.');if(stop===null)warnings.push('Geçerli stop belirlenemedi; stop uyarısı olmayacak.');if(atTarget!==null&&atTarget<=0)warnings.push('Hedef getirisi tahmini maliyetleri karşılamıyor.');return{entry,quantity,target,stop,risk,riskCap,stopAdjusted,atTarget,atStop,warnings};}
function previewTrade(){if(!state.selected)return;try{const inputs=tradeInputs(),plan=tradePlan(state.selected,inputs);byId('trade-target').textContent=levelText(plan.target,plan.entry);byId('trade-stop').textContent=levelText(plan.stop,plan.entry);byId('trade-risk').textContent=frameName(byId('trade-frame').value)+' planı · Tahmini giriş '+money(plan.entry)+' · Hedefte net '+money(plan.atTarget)+' · Stopta net '+money(plan.atStop)+' · Net getiri/risk '+(num(plan.atTarget)&&num(plan.atStop)?(plan.atTarget/Math.abs(plan.atStop)).toFixed(2):'—');byId('trade-message').textContent=(plan.warnings.length?'⚠ '+plan.warnings.join(' ')+' ':'')+'Sanal işlem açabilirsin; risk uyarıları engel değildir. Son onayda güncel fiyat alınır.';}catch(e){byId('trade-risk').textContent='';byId('trade-message').textContent=e.message;}}
let tradeGeneration=0;
let tradeSymbol=null;
async function refreshTrade(){
  if(submitting||!tradeSymbol)return;
  const generation=++tradeGeneration,frame=byId('trade-frame').value;
  byId('trade-message').textContent=frameName(frame)+' hedef/stop planı ve güncel fiyat alınıyor…';byId('trade-submit').disabled=true;state.selected=null;
  byId('trade-entry').textContent='';byId('trade-target').textContent='—';byId('trade-stop').textContent='—';byId('trade-risk').textContent='';
  try{const d=await api('/api/analyze',{symbol:tradeSymbol,frames:frame,fresh:'1'});
    if(generation!==tradeGeneration||!byId('trade-dialog').open)return;
    if(d.item.frame_keys.length!==1||d.item.frame_keys[0]!==frame)throw Error('Seçilen periyot doğrulanamadı; yeniden dene.');
    state.selected=d.item;rememberQuote(d.item.coin);byId('trade-entry').textContent='Borsa son fiyatı: '+money(d.item.coin.price)+' · '+timeText(d.item.coin.price_updated_at);byId('trade-submit').disabled=false;previewTrade();
  }catch(e){if(generation===tradeGeneration)byId('trade-message').textContent=e.message+' Planı yenile düğmesiyle tekrar deneyebilirsin.';}
}
async function openTrade(s){
  if(submitting)return;tradeSymbol=s.coin.symbol;byId('trade-title').textContent=tradeSymbol;byId('trade-amount').value='1000';
  byId('trade-frame').replaceChildren(...catalog.map(([key,name])=>{const option=el('option','',name);option.value=key;return option;}));
  byId('trade-frame').value=catalog.some(([key])=>key===s.chart_frame)?s.chart_frame:'one_hour';
  openDialogAtTop(byId('trade-dialog'));await refreshTrade();
}
let submitting=false;
async function submitTrade(event){event.preventDefault();try{await portfolioLock(()=>{syncStoredPortfolio();return submitTradeUnlocked(event);});}catch(e){byId('trade-message').textContent=e.message;}}
async function submitTradeUnlocked(event){event.preventDefault();if(submitting||!state.selected)return;submitting=true;byId('trade-submit').disabled=true;byId('trade-frame').disabled=true;byId('trade-refresh').disabled=true;try{const inputs=tradeInputs(),selected=state.selected,frame=byId('trade-frame').value;byId('trade-message').textContent='Giriş fiyatı son kez doğrulanıyor…';const d=await api('/api/analyze',{symbol:selected.coin.symbol,frames:frame,fresh:'1'}),s=d.item;if(s.frame_keys.length!==1||s.frame_keys[0]!==frame)throw Error('Seçilen periyot doğrulanamadı; işlem açılmadı.');const plan=tradePlan(s,inputs);rememberQuote(s.coin);const p={id:crypto.randomUUID(),coin:{symbol:s.coin.symbol},amount:inputs.amount,quantity:plan.quantity,entry:plan.entry,target:plan.target,stop:plan.stop,riskWarning:plan.warnings.join(' '),extendedTarget:s.extended_target,fee:inputs.fee,slippage:inputs.slip,openedAt:new Date().toISOString(),frames:[frame]};savePortfolio([p,...state.positions],state.history);byId('trade-dialog').close();renderPortfolio();switchPage('home');notice('Sanal pozisyon açıldı · '+frameName(frame)+'. Gerçek borsa emri gönderilmedi.'+(plan.warnings.length?' ⚠ '+plan.warnings.join(' '):''));}catch(e){byId('trade-message').textContent=e.message;}finally{submitting=false;byId('trade-submit').disabled=false;byId('trade-frame').disabled=false;byId('trade-refresh').disabled=false;}}
function closePosition(id){openPartialSale(id);}
...
async function importBackup(file){try{await portfolioLock(()=>{syncStoredPortfolio();return importBackupUnlocked(file);});}catch(e){byId('backup-message').textContent=e.message;}}
async function importBackupUnlocked(file){if(!file)return;try{if(file.size>2*1024*1024)throw Error('Yedek en fazla 2 MB olabilir.');if(brokenStorage.size)throw Error('Önce mevcut bozuk kayıtların ham yedeğini indir; içe aktarma güvenlik için durduruldu.');const d=parseBackup(await file.text());const merged=Paper.mergePortfolio({positions:state.positions,history:state.history},d);savePortfolio(merged.positions,merged.history);state.favorites=[...new Set([...state.favorites,...d.favorites])];store(keys.favorites,state.favorites);renderPortfolio();renderFavorites();byId('backup-message').textContent='Yedek birleştirildi. Parçalı satışların kalan miktarı korundu; kapalı işlemler yeniden açılmadı. Eski miktara ait bekleyen emir iptal edilmiş olabilir.';}catch(e){byId('backup-message').textContent=e.message;}finally{byId('import-backup').value='';}}
...
function notifyExit(p,data){
  for(const [frameKey,frame] of Object.entries(data.frames)){
    alertOnce('exit:'+exitKey(p)+'|'+frameKey,frame.count>=3?'qualified':'',symbolOf(p)+' · SATIŞ UYARISI · '+frame.name+' '+frame.count+'/4'+(data.errors.length?' · Bazı periyotlar alınamadı.':''),'sell','sell');
  }
}
...
async function loadExitChecks(force=false){
  ...
  try{await Promise.all([worker(),worker()]);}finally{exitBusy=false;await checkAutoSales();renderPortfolio();}
}
...
byId('trade-frame').addEventListener('change',refreshTrade);
byId('trade-refresh').addEventListener('click',refreshTrade);
