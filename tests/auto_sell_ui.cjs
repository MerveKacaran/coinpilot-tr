...
(async()=>{
  ...
  await context.route('**/api/**',async route=>{
    ...
    else if(u.pathname==='/api/targets')d={...d,symbol:'TEST/TRY',frames:{fifteen_minute:{target:99,stop:90},one_hour:{target:105,stop:90},daily:{target:120,stop:90}},errors:[],analyzed_at:iso(now())};
  ...
  assert.equal(await page.locator('#page-positions [data-position-id="stop"] .stop-reached').count(),3);
  ...
  starts=await page.evaluate(()=>window.__audioEvents.filter(x=>x.event==='start'));
  assert.equal(starts.length,60);assert.equal(starts[40].type,'sawtooth');assert.ok(starts.at(-1).at-starts[40].at>=9.5);
  await page.evaluate(()=>playSignalAlarm('buy'));starts=await page.evaluate(()=>window.__audioEvents.filter(x=>x.event==='start'));
  assert.equal(starts.length,80);assert.equal(starts[60].type,'sine');assert.ok(starts.at(-1).at-starts[60].at>=9.5);
  assert.deepEqual(errors,[]);
  console.log('PASS: target/stop status, distinct 10-second buy/sell/target/stop alarms, opt-in automatic paper sales');
  await browser.close();
