...
async function main(){const browser=await chromium.launch({headless:true,executablePath:process.env.CHROME_PATH||'C:/Program Files/Google/Chrome/Application/chrome.exe'});testBrowser=browser;const context=await browser.newContext({viewport:{width:1440,height:1000},acceptDownloads:true});const page=await context.newPage(),errors=[];page.on('pageerror',e=>errors.push(e.message));
  ...
  await page.locator('[data-page=exits]').click();await page.locator('#exit-grid .exit-details summary').first().click();assert.match(await page.locator('#exit-grid').textContent(),/MACD sıfır üstünde/);
  ...
  assert.equal(await page.locator('#radar-positions').count(),0,'Technical Radar does not duplicate open positions');await page.locator('[data-page=radar]').click();assert.equal(await page.locator('#page-radar .position').count(),0);assert.ok(await page.locator('#radar-grid .signal').count()>0,'Radar candidates remain visible');
  await page.locator('[data-page=positions]').click();assert.match(await page.locator('#position-page-list').textContent(),/Tahmini net K\/Z/);assert.match(await page.locator('#position-page-list').textContent(),/Hedef.*114.*%.*Stop.*-.*%/);assert.match(await page.locator('#position-page-list').textContent(),/Plan periyodu: 15 Dakika/);for(const id of ['home-positions','position-page-list','exit-grid'])assert.equal((await page.locator('#'+id).evaluate(n=>getComputedStyle(n).gridTemplateColumns)).split(' ').length,1,id+' must be one wide column');
  await page.waitForFunction(()=>document.querySelector('#position-page-list .position-mini-chart canvas')?.width>0);assert.match(await page.locator('#position-page-list .position-mini-chart').first().textContent(),/15 Dakika FİYAT GRAFİĞİ.*Mavi giriş.*Yeşil hedef.*Kırmızı stop.*Sarı canlı fiyat/s);const positionChartPixels=await page.locator('#position-page-list .position-mini-chart canvas').first().evaluate(c=>{const a=c.getContext('2d').getImageData(0,0,c.width,c.height).data;let n=0;for(let i=3;i<a.length;i+=4)if(a[i])n++;return n;});assert.ok(positionChartPixels>500,'Position candlestick chart rendered');
  await page.locator('#position-page-list .position-chart-actions select').first().selectOption('daily');await page.waitForFunction(()=>document.querySelector('#position-page-list .position-mini-chart b')?.textContent.includes('Günlük'));assert.equal(await page.locator('#position-page-list .position-chart-actions select').first().inputValue(),'daily');await page.evaluate(()=>loadDashboard());assert.equal(await page.locator('#position-page-list .position-chart-actions select').first().inputValue(),'daily','Chart timeframe survives live card rerender');
  await page.locator('#position-page-list .position-indicators summary').first().click();assert.equal(await page.locator('#position-page-list .position-indicators').first().evaluate(n=>n.open),true);assert.match(await page.locator('#position-page-list .position-indicators').first().textContent(),/EMA8 \/ EMA200.*RSI\(10\).*MACD.*Fisher\(30\).*Hacim/s);
  await page.waitForFunction(()=>document.querySelector('#position-page-list .position-indicators canvas')?.width>0);const indicatorPixels=await page.locator('#position-page-list .position-indicators canvas').first().evaluate(c=>{const a=c.getContext('2d').getImageData(0,0,c.width,c.height).data;let n=0;for(let i=3;i<a.length;i+=4)if(a[i])n++;return n;});assert.ok(indicatorPixels>500,'Position technical indicators rendered');
  await page.locator('#position-page-list .position-indicator-controls input[value=volume]').first().uncheck();assert.equal(await page.locator('#position-page-list .position-indicator-controls input[value=volume]').first().isChecked(),false);await page.evaluate(()=>loadDashboard());assert.equal(await page.locator('#position-page-list .position-indicators').first().evaluate(n=>n.open),true,'Indicator details remain open after live refresh');assert.equal(await page.locator('#position-page-list .position-indicator-controls input[value=volume]').first().isChecked(),false,'Indicator choice survives live refresh');
  ...
  await page.locator('[data-page=performance]').click();assert.match(await page.locator('#page-performance').textContent(),/Gerçekleşen net K\/Z.*İşlem performansı.*TEST\/TRY.*Gerçekleşen sanal emir geçmişi/si);assert.equal(await page.locator('#performance-completed tr').count(),1);assert.match(await page.locator('#completed-order-count').textContent(),/1 kayıt/);
  ...
  for(const surface of [page,mobile]){
    for(const destination of ['market','radar','watchlist','positions','performance','exits','home']){
